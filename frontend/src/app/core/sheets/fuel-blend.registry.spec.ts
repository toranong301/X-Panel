...
describe('FuelBlendRegistry', () => {
  it('computes B7 blend totals (540 L)', () => {
    ...
    expect(result.dieselL).toBeCloseTo(502.2, 6);
    expect(result.biodieselL).toBeCloseTo(37.8, 6);
    expect(result.biodieselKg).toBeCloseTo(32.886, 6);
  });

  it('computes 91/95 blend totals (51.546 L)', () => {
    const result = computeBlendFromAnnualL(51.546, '91/95');
    expect(result.gasolineL).toBeCloseTo(46.3914, 6);
    expect(result.ethanolL).toBeCloseTo(5.1546, 6);
    expect(result.ethanolKg).toBeCloseTo(4.072134, 6);
  });
